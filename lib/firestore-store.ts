...
export function createFirestoreStore(database: Firestore) {
  ...
  async function load(tx: Transaction, context: StoreContext): Promise<Session> {
    ...
    if (health || (!orgId && !state.admin)) {
      ...
      if (context.reportCampaign) await referenced(tx,'campaigns',[context.reportCampaign],snapshot.campaigns);
      if (context.reportScreen && actor?.role !== 'advertiser_viewer') await referenced(tx,'screens',[context.reportScreen],snapshot.screens);
      let q: any = database.collection('screen_day').where('id','>=',range.lower).where('id','<',range.upper);
      if (actor?.role === 'advertiser_viewer') q = q.where('advertiser_id','==',actor.advertiser_id || '__none__');
      else if (scoped) q = q.where('org_id','==',orgId);
      if (context.reportScreen) q = q.where('screen_id','==',context.reportScreen);
      if (context.reportCampaign) q = q.where('campaign_id','==',context.reportCampaign);
      if (context.after) q = q.where('id','>',context.after);
      const result: any = await tx.get(q.orderBy('id').limit(REPORT_PAGE_SIZE + 1));
      const rows = result.docs.map((d: any) => d.data());
      ...
      if (device && context.assignmentId && context.startedAtDevice) {
        ...
        if (a && Number.isFinite(at) && context.path[0] === 'play') {
          // Include receive-day neighbours: invalid clock reports use receive time, which may
      ...
      if (actor?.role === 'advertiser_viewer') await referenced(tx,'screens',snapshot.settlement_buckets.map((b: any) => b.screen_id),snapshot.screens);
      ...
      if (!deviceRequest && context.method === 'GET' && ['screen', 'campaign'].includes(context.path[0])) {
  ...
  async function flush(state: Session) {
    ...
    if (!original.reporting_coverage && snapshot.reporting_coverage && changes.some(c => c.collection === 'screen_day'))
      tx.create(doc('_meta','reporting'),clean(snapshot.reporting_coverage));
