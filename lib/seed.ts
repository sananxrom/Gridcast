...
export function seed() {
  ...
  for (const c of campaigns as any[]) {
    c.participant_org_ids = [...new Set(c.screen_ids.map((id: string) => screens.find(s => s.id === id)!.org_id))];
    if (c.campaign_type === 'network') c.bookings = c.screen_ids.map((screen_id: string) => ({screen_id,slots_per_loop:1,reserved_slot_units:1,booked_at:nowISO()}));
  }
  const groups = [
