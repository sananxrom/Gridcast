...
**Status (2026-09-27):** B–E plus the optional-calibration correction are reviewed and deployed to Firebase as application commit `e6af562a30fafac4e47fc2094254ec9e80f9301e`, player `gridcast-web/0.9.1`, rollout `build-2026-09-26-003` (READY/SUCCEEDED, 100% traffic). Calibration is optional; compatible default/guided results share one report with truthful per-play provenance. Current release verification: 305 unit passes (5 emulator-only skips), 45 browser passes before the final per-play binding check plus its focused rotation regression, repeated default-mode Enterprise emulator aggregation, TypeScript and production build. Live health/player/version, model hashes and both production lab 404s verified. Previous V1 release `023e4f4` also passed 25 emulator checks and real pinned cold/offline inference; it remains the rollback. Attention remains off until enabled per screen; no fleet activation or live analytics receipt was manufactured. Sanan authorized advancing beyond the originally proposed A2 gate; human accuracy, representative-hardware performance and soak validation remain open before pilot promotion. See [26 — contracts and validation](26-attention-contracts-and-validation.md). **Planning base:** `7d5f085`.
