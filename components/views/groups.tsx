...
export function GroupManager({boot,user,orgId,onChanged}:{boot:any;user:any;orgId?:string|null;onChanged:()=>void}) {
  ...
  const open=(g?:any)=>{setError('');setDraft({id:g?.id,org_id:g?.org_id??orgId??(user.role==='platform_admin'?'':user.org_id),name:g?.name??'',group_type:g?.group_type??'static',screen_ids:[...(g?.screen_ids??[])],venue_types:(g?.rule_json?.venue_types??[]).join(', '),min_size:g?.rule_json?.min_size??'',location_tier:g?.rule_json?.location_tier??'',city:g?.rule_json?.city??'',area:g?.rule_json?.area??'',tags:Object.entries(g?.rule_json?.tags??{}).map(([k,v])=>`${k}:${v}`).join(', ')});};
  const rule=():any=>{
  ...
  const save=async()=>{setError('');setBusy(true);try{if(!draft.org_id)throw new Error('Select an organisation first.');if(!draft.name.trim())throw new Error('Enter a group name.');await api(draft.id?`/group/${draft.id}`:'/group',{org_id:draft.org_id,name:draft.name.trim(),group_type:draft.group_type,screen_ids:draft.group_type==='static'?draft.screen_ids:[],rule_json:draft.group_type==='dynamic'?rule():null});setDraft(null);await onChanged();}catch(e){setError((e as Error).message);}finally{setBusy(false);}};
  ...
  return <><PageHead title="Screen groups" sub="Reusable screen selections. Campaigns keep the screens selected when you book them." actions={mayEdit?<Button onClick={()=>open()}>Create group</Button>:undefined}/>
    {draft&&<Card className="mb-4 p-5"><h3 className="mb-3 font-semibold">{draft.id?'Edit':'Create'} group</h3><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {user.role==='platform_admin'&&<Field label="Organisation"><Select disabled={!!draft.id||!!orgId} value={draft.org_id} onChange={e=>setDraft({...draft,org_id:e.target.value,screen_ids:[]})}><option value="">Choose organisation…</option>{boot.orgs.map((o:any)=><option key={o.id} value={o.id}>{o.name}</option>)}</Select></Field>}
