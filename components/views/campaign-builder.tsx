...
export function CampaignBuilder({ boot, user, onGo, onDone }: {
  ...
  const availableAdvertisers=boot.advertisers.filter((a:any)=>user.role==='platform_admin'||a.org_id===user.org_id);
  ...
  const selectedOrg = advertisers.find((a:any)=>a.id===advId)?.org_id ?? user.org_id;
  ...
  const addAdvertiser=async()=>{setErr('');try{if(!newAdv.name.trim()) throw new Error('Enter the advertiser name.');const a=await api('/advertiser',{org_id:user.org_id,...newAdv,category:'general'});setAddedAdvertisers([...addedAdvertisers,a]);setAdvId(a.id);}catch(e){setErr((e as Error).message);}};
  ...
  return (
    <>
      ...
      <Card className="mb-4 p-5">
        ...
        <div className="flex flex-wrap gap-3">
          <Field label="Advertiser">
            <Select value={advId} onChange={e => {setAdvId(e.target.value);setCreatives([]);setScreens([]);}}>
      ...
      <div className="flex items-center gap-3">
        <Field label="Initial status" className="max-w-[220px]"><Select value={status} onChange={e=>setStatus(e.target.value)}><option value="pending">Pending — reserve capacity</option><option value="draft">Draft — no reservation</option><option value="active">Active — approved creatives play</option></Select></Field><Button onClick={save} disabled={saving}>{saving?'Creating…':'Create campaign'}</Button>
