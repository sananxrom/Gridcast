...
const setCampaign=async(c:any,patch:any)=>{await api(`/campaign/${c.id}`,patch);await reload();};
...
const campaignBulk:BulkAction<any>[]=['paused','active'].map(status=>({label:status==='active'?'Resume':'Pause',run:async rows=>{for(const c of rows)await api(`/campaign/${c.id}`,{status});}}));
return (
<>
        ...
        <DataTable cols={[
          ...
          ...(caps.includes('money') ? [{ label: 'Invoice', sort: (c: any) => c.invoice_status, render: (c: any) => (
            <InlineSelect disabled={!caps.includes('money')} value={c.invoice_status} choices={INVOICE_CHOICES} onChange={(v:string) => setCampaign(c, { invoice_status: v })}>
          ...
          facets={[{ label: 'Status', get: (c: any) => c.status }, { label: 'Type', get: (c: any) => c.campaign_type },
                   ...(caps.includes('money') ? [{ label: 'Invoice', get: (c: any) => (c.invoice_status??'not_invoiced').replace(/_/g, ' ') }] : [])]} />
      </>
);
}
