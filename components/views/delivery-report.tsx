...
export function dailyReportCsv(data: DeliveryData, period: Period) {
  const header = ['date_ist', 'period_from', 'period_to', 'coverage_started_at', 'period_fully_covered', 'source', 'paid_delivered', 'paid_billable', 'paid_failed', 'filler_receipts', 'presence_sum', 'measured_paid_plays', 'mean_people_per_measured_paid_play', 'paid_airtime_ms', 'invalid_clock_receipts','profile_series_id','profile','manifest_sha256','pipeline_sha256','calibration_provenance','calibrated_attention_observed_share','asset_versions','attention_plays','playing_ms','attention_observed_ms','attention_unknown_ms','looking_person_ms','face_assessable_person_ms','estimated_impressions','attentive_impressions'];
  const rows = rangeDays(period).map(date => {
    ...
    return [date, period.from, period.to, data.coverage.started_at, data.coverage.complete, 'accepted_device_receipts_daily_aggregate', row?.plays_rendered, row?.plays_billable, row?.plays_not_rendered, row?.plays_filler, row?.presence_n ? row.presence_sum : null, row?.presence_n, row ? average(row) : null, row?.airtime_ms, row?.plays_time_invalid ?? (row ? 0 : null),...Array(15).fill(null)];
  });
  for(const [key,p] of Object.entries(data.attentionProfiles||{}))for(const [date,c]of Object.entries(p.daily||{})){const prov=Object.values(p.provenance||{}),dayProv=prov.filter(x=>(x.daily?.[date]?.plays||0)>0),dayGuided=dayProv.filter(x=>x.mode==='guided').reduce((n,x)=>n+(x.daily?.[date]?.attention_observed_ms||0),0),dayObserved=c.attention_observed_ms||0;const provenance=dayProv.map(x=>`${x.mode}${x.calibration_revision?':'+x.calibration_revision:''}`).join('|');const assets=Object.values(p.byCreativeAsset||{}).filter(x=>(x.daily?.[date]?.plays||0)>0).length;rows.push([date,period.from,period.to,data.coverage.started_at,data.coverage.complete,'attention_profile_series',null,null,null,null,null,null,null,null,null,key,p.profile,p.manifest_sha256,p.pipeline_sha256,provenance,dayObserved?dayGuided/dayObserved:null,assets,c.plays,c.playing_ms,c.attention_observed_ms,c.attention_unknown_ms,c.attention_observed_ms?c.looking_person_ms:null,c.attention_observed_ms?c.face_assessable_person_ms:null,c.body_observed_ms?c.estimated_impressions:null,c.attention_observed_ms?c.attentive_impressions:null]);}
...
export function DeliveryReport({ report, screens = [], campaigns = [], creatives = [] }: { report: DeliveryReportState; screens?: { id: string; name: string }[]; campaigns?: { id: string; name: string }[]; creatives?: { id: string; name: string }[] }) {
  ...
  return <section className="space-y-4" aria-label="Delivery report" aria-busy={loading}>
    ...
    {data && !loading && <>
      ...
      <Card className="p-4" aria-label="Attention analytics"><div className="flex flex-wrap items-center gap-3"><h3 className="text-sm font-semibold">Attention insights</h3>{attentionKeys.length>1&&<Select aria-label="Attention model profile" className="ml-auto w-auto" value={selectedAttentionKey} onChange={e=>setAttentionSeries(e.target.value)}>{attentionKeys.map(key=>{const p=data.attentionProfiles[key];return <option key={key} value={key}>Attention V1 · {p.profile}</option>;})}</Select>}</div>
