...
const APP_VERSION = 'gridcast-web/0.3.0';
const MODEL_VERSION = 'coco-ssd@2.2.3/lite_mobilenet_v2';
type Credential = { token: string; device_id: string; screen_id: string };
type Item = { assignment_id: string; valid_until: string; campaign_id: string; creative_id: string; creative_name: string; advertiser: string;
  youtube_id?: string; asset_url?: string; asset_id?: string; asset_mime?: string; width?: number; height?: number; duration_s: number };
type Playlist = { screen: any; config: Record<string, any>; config_version: string | number; server_time: string; items: Item[] };
type Slot = { item: Item; uid: string; started: number; startedPlaying: boolean; accumulated: number; segmentStart: number | null;
  mediaStart: number; samples: number[]; cameraHealthy: boolean; config: Record<string, any>; version: string | number; offset: number; done: boolean };
...
export default function Player() {
  ...
  useEffect(() => {
    ...
    function segment(playing: boolean) {
      ...
      if (playing && s.segmentStart === null) {
        ...
        s.segmentStart = performance.now(); state('Playing');
        void authRequest('/nowplaying', { assignment_id: s.item.assignment_id }).catch(() => {});
    ...
    function cameraHealthyNow() {
      const v = camera.current, tracks = stream?.getVideoTracks() || [];
    ...
    function cameraLost() { cameraOK = false; if (currentSlot) currentSlot.cameraHealthy = false; if (!disposed) setCount(null); }
    function cameraRecovered() { cameraOK = cameraHealthyNow(); }
    ...
    async function finish(reason: 'ended' | 'duration_observed' | 'error' | 'timeout' | 'interrupted') {
      const s = currentSlot; if (!s || s.done) return;
      segment(false); const mediaEnd = mediaTime(); s.done = true; currentSlot = null; stopMedia(); if (!disposed) setCurrent(null);
    ...
    async function pull() {
      ...
      try {
        ...
        pending = d; lastPull = Date.now(); if (!disposed) setScreen(d.screen);
    ...
    async function startNext() {
      ...
      try {
        ...
        if (!cycleActive) {
          ...
          if (!cycleItems.length) { waitingEmpty = true; setCurrent(null); state(playlist.items.length ? 'Playlist expired. Waiting for connection.' : 'Waiting for an eligible campaign'); return; }
    ...
    async function heartbeat() {
      ...
      try {
        await authRequest('/heartbeat', { device_now: new Date().toISOString(), app_ver: APP_VERSION, agent_ver: detector ? MODEL_VERSION : 'camera-unavailable',
          config_version: playlist.config_version, uptime_s: Math.round((performance.now() - booted) / 1000), free_disk_bytes: null });
    ...
    async function initCamera() {
      try {
        ...
        if (disposed || !camera.current) return;
        // Asset packaging is supplied by the deployment; no remote model/CDN fallback.
        detector = await coco.load({ base: 'lite_mobilenet_v2', modelUrl: '/models/coco-ssd/model.json' });
        ...
        camera.current.srcObject = stream; await camera.current.play(); cameraRecovered();
      } catch { cameraOK = false; if (!disposed) setCount(null); }
    }
    async function detect() {
      const v = camera.current, c = canvas.current;
      if (disposed || detecting || !detector) return;
      cameraOK = cameraHealthyNow();
      if (!v || !c || !cameraOK) { cameraLost(); return; }
      detecting = true; const target = currentSlot;
      try {
        const k = target?.config || playlist?.config || {};
        ...
        const all = await detector.detect(v, ceiling, confidence);
        if (!cameraHealthyNow()) { cameraLost(); return; }
        const zone = k.detection_zone || { x: 0, y: 0, w: 100, h: 100 };
        const persons = all.filter((p: any) => {
          const [x, y, w, h] = p.bbox, cx = (x + w / 2) / v.videoWidth * 100, cy = (y + h / 2) / v.videoHeight * 100;
          return p.class === 'person' && p.score >= confidence && h >= Number(k.min_box_px ?? 24) && cx >= zone.x && cx <= zone.x + zone.w && cy >= zone.y && cy <= zone.y + zone.h;
        });
        const n = Math.min(persons.length, ceiling);
        if (!disposed) setCount(n);
        if (target && currentSlot === target && target.segmentStart !== null && !target.done) target.samples.push(n);
        c.width = v.videoWidth; c.height = v.videoHeight;
        const ctx = c.getContext('2d'); if (ctx) { ctx.clearRect(0, 0, c.width, c.height); ctx.strokeStyle = '#F59E0B'; ctx.lineWidth = 3; for (const p of persons) ctx.strokeRect(...p.bbox as [number, number, number, number]); }
      } catch { if (target) target.cameraHealthy = false; if (!disposed) setCount(null); }
    ...
    const visibility = () => { if (document.hidden) void finish('interrupted'); else void startNext(); };
    ...
    const ticker = setInterval(() => {
      ...
      if (Date.now() - lastPull > Math.min(300, Math.max(30, (Number(playlist?.config.sync_interval_min) || 5) * 60)) * 1000) { lastPull = Date.now(); void pull(); }
    ...
    void pull().then(() => { if ((pending || playlist)?.screen.has_camera) void initCamera(); });
    return () => {
      ...
      stream?.getTracks().forEach(t => { t.removeEventListener('ended', cameraLost); t.removeEventListener('mute', cameraLost); t.removeEventListener('unmute', cameraRecovered); t.stop(); }); detector?.dispose?.(); yt?.destroy?.();
    };
  }, [credential]);

  const exportQueue = async () => {
    try {
      const records = await queuedPlays();
  ...
  if (!credential) return <div className="grid min-h-screen place-items-center bg-slate-950 p-4"><div className="w-full max-w-[420px] rounded-xl bg-card p-8">
    ...
    <p className="mt-4 text-xs text-muted-foreground">Camera processing and the setup preview stay on this device. Only presence counts and delivery records are sent. Playback reports are buffered during connection failures; YouTube videos still require internet.</p>
  </div></div>;
  return <div className="fixed inset-0 bg-black text-white">
    ...
    <div className="fixed left-3 top-3 z-10 rounded bg-black/80 p-3 text-xs"><b>{screen?.name || 'Gridcast'}</b> · {current?.creative_name || 'Waiting'}<br />{status}
    ...
    <div className="fixed bottom-3 right-3 z-10 w-[170px]"><div className="relative"><video ref={camera} autoPlay muted playsInline className="w-full rounded bg-neutral-900" /><canvas ref={canvas} className="pointer-events-none absolute inset-0 h-full w-full" /></div><p className="mt-1 text-[10px] text-white/60">Local camera preview · never uploaded</p></div>
    <div className="fixed bottom-3 left-3 z-10 rounded bg-black/80 p-3 text-xs">People now: {count ?? '—'} · Last play average: {average}<br />Recorded this session: {recorded} · Pending delivery: {queue.pending}
