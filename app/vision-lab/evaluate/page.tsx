...
export default function AttentionEvaluation() {
  ...
  const [mode, setMode] = useState<'idle' | 'camera' | 'simulation'>('idle');
    ...
    try {
      if (!simulation) {
        ...
        engine = await createEvaluationWorker(delegate, controller.signal, (observation, nextStats) => {
          if (current()) { metrics.current.observe(observation); setStats(nextStats); }
        ...
        check = setInterval(async () => {
          ...
          try { await assertIsolated(); } catch (e: any) { if (!controller.signal.aborted) { runMetadata.current?.warnings.push('Another player tab was detected; isolation was interrupted.'); stop('Stopped to keep this benchmark isolated.'); setError(e.message); } } finally { checking = false; }
        }, 2500);
      }
      runMetadata.current = { mode: simulation ? 'simulation' : 'camera', delegate, calibration: {...calibration}, started_at: new Date().toISOString(), warnings: [] };
  ...
  const summary = snapshot?.current || snapshot?.completed.at(-1), live = mode === 'idle' ? undefined : snapshot?.live;
  ...
  return <main className="min-h-screen bg-slate-50 p-4 text-slate-900 sm:p-8"><div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="rounded-xl border bg-white p-5"><div className="flex flex-wrap items-end gap-3">
      ...
      <button className="ml-auto rounded-lg border px-4 py-2 text-sm disabled:opacity-40" disabled={!summary} onClick={download}>Download aggregate results</button>
