...
export default function AttentionEvaluation() {
  ...
  return <main className="min-h-screen bg-slate-50 p-4 text-slate-900 sm:p-8"><div className="mx-auto max-w-6xl space-y-6">
    ...
    <header className="flex flex-wrap items-start justify-between gap-4"><div><BrandLogo compact className="mb-4"/><h1 className="text-3xl font-semibold tracking-tight">Attention lab</h1><p className="mt-2 max-w-2xl text-sm text-slate-600">Test presence, looking and visit duration on this computer. Evaluation results stay in this tab and do not affect campaigns, delivery or billing.</p>{process.env.NODE_ENV !== 'production' && <a className="mt-2 inline-block text-sm font-medium text-amber-800 underline" href="/vision-lab/combined">Open the local A2 combined-workload comparison →</a>}</div><span className="rounded-full border bg-white px-3 py-1 text-xs">{online ? 'Online' : 'Offline'} · {mode === 'simulation' ? 'Simulation' : 'Local evaluation'}</span></header>
