...
test('guided calibration applies offsets, preserves prior evidence, and retains settings on failure or cancel',{skip:skip||(!process.env.GC_TEST_REAL_CDN?'Set GC_TEST_REAL_CDN=1 for preparation':false),timeout:180000},async t=>{
 const h=await setup(t),p=h.page;await p.getByRole('button',{name:'Try simulation'}).click();await p.waitForTimeout(400);await p.getByRole('button',{name:'Stop',exact:true}).click();const before=await exportData(p);
 await p.getByRole('button',{name:'Prepare offline files'}).click();await p.getByRole('status').filter({hasText:'Model files verified and cached'}).waitFor({timeout:120000});
 // Only UI lifecycle uses synthetic worker outputs here; the separate runtime test executes real models.
 await p.evaluate(()=>{window.calibrationCase='valid';window.Worker=class{terminated=false;postMessage(data){if(data.type==='INIT')setTimeout(()=>{if(!this.terminated)this.onmessage?.({data:{type:'READY'}});},0);else if(data.type==='FRAME'){data.frame.close();const valid=window.calibrationCase==='valid';setTimeout(()=>{if(!this.terminated)this.onmessage?.({data:{type:'OBSERVATION',durationMs:1,observation:{at:data.at,faces:{ok:true,saturated:false,faces:valid?[{box:[.3,.2,.5,.4],looking:false,smiling:false}]:[]},calibration:valid?{yaw:8,pitch:-3.5}:null}}});},0);}}terminate(){this.terminated=true;}};});
 await p.getByRole('button',{name:'Auto-calibrate (3 s)'}).click();await p.getByRole('dialog').waitFor();await p.getByRole('status').filter({hasText:'Calibration complete.'}).waitFor();await p.locator('summary').click();assert.equal(await p.getByLabel('yaw offset').inputValue(),'8');assert.equal(await p.getByLabel('pitch offset').inputValue(),'-3.5');
 const after=await exportData(p);assert.deepEqual(after.results.completed,before.results.completed);assert.deepEqual(after.calibration,before.calibration);assert.equal(after.calibration_method,before.calibration_method);assert.deepEqual(after.performance,before.performance);
 await p.evaluate(()=>window.calibrationCase='empty');await p.getByRole('button',{name:'Auto-calibrate (3 s)'}).click();await p.getByRole('alert').filter({hasText:'Not enough clear face readings'}).waitFor();assert.equal(await p.getByLabel('yaw offset').inputValue(),'8');
 await p.getByRole('button',{name:'Auto-calibrate (3 s)'}).click();await p.getByRole('button',{name:'Cancel calibration'}).click();await p.waitForTimeout(3300);assert.equal(await p.getByLabel('yaw offset').inputValue(),'8');assert.equal(await p.getByRole('dialog').count(),0);
 await p.getByRole('button',{name:'Try simulation'}).click();await p.waitForTimeout(300);const fresh=await exportData(p);assert.equal(fresh.calibration_method,'guided-3s');assert.deepEqual(fresh.calibration,{yaw:8,pitch:-3.5});await p.getByRole('button',{name:'Stop',exact:true}).click();assert.deepEqual(h.errors,[]);assert.deepEqual(h.api,[]);
});
